...
import {Location, LocationChangeEvent, LocationStrategy, PathLocationStrategy} from '@angular/common';
...
export class BoardViewComponent implements OnInit {
  ...
  constructor(private _route : ActivatedRoute, private _router : Router, location : Location) {
    this.location = location;
    this
      .location
      .subscribe((event : LocationChangeEvent) => {
  ...
  initGame() {
    ...
    const moveno = +this
      ...
      .paramMap
      .get('moveno');
    // console.log('moveno:', moveno); Show curr move no in pagination
  ...
  private _getPreviousMoves() {
    ...
    this.detail.next_turn = this.detail.moves_json[this.detail.currentmove] || null;
    let t = _.findLastIndex(_.first(this.detail.moves_json, move_no), {player: 1});
    this.detail.player1.prev_turn = this.detail.moves_json[t];
    t = _.findLastIndex(_.first(this.detail.moves_json, move_no), {player: 2});
    this.detail.player2.prev_turn = this.detail.moves_json[t];
    if (this.detail.turn['event'] === 'chall+') {
      this.detail.turn['fullword'] = this.detail.moves_json[this.detail.currentmove - 2]['fullword'];
    }
    // Get the scores at the end of the game. End of the game being when extra tiles
    // are added or when there is time penalty
    if (this.detail.turn['event'] === 'rack+' || this.detail.turn['event'] === 'time+') {
      let m = _.findLastIndex(_.first(this.detail.moves_json, this.detail.currentmove), {player: 1});
      this.detail.player1.final_score = this.detail.moves_json[m]['totalscore'];
      m = _.findLastIndex(_.first(this.detail.moves_json, this.detail.currentmove), {player: 2});
  ...
  private _pageChange(moveno : number) : void {
    ...
    this.detail.turn = this.detail.moves_json[moveno - 1];
    this._getPreviousMoves();
    // this.detail.next_turn = this.detail.moves_json[moveno] || null;
    // this.detail.prev_turn = this.detail.moves_json[moveno - 2] || null; const ev
    // = this.detail.turn['event']; console.log('This event: ' + ev); if (ev ===
    // 'chall+') {   this.detail.turn = this.detail.moves_json[moveno - 1];
    // this.detail.turn['fullword'] = this.detail.moves_json[moveno -
    // 2]['fullword'];   this.detail.prev_turn = this.detail.moves_json[moveno - 3];
    // }
    this
      .moveboard
      .showMove(moveno);
